...
while not input_data == "Stop":
    ...
    if current_command[0] == "Delete":
        ...
        index_to_use = int(current_command[2])
        if index_to_use in range(len(input_string)) and index_to_use - 1 in range(len(input_string)):
            if index_to_use == current_command[-1]:
                input_string.append(word_to_add)
            else:
                input_string.insert(index_to_use - 1, word_to_add)
    elif current_command[0] == "Sort":
        input_string.sort(reverse=False)
